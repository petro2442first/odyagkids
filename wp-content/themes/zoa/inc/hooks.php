...
function zoa_backups_demos( $demos ) {
	$demos_array = array(
		'zoa' => array(
			...
			'preview_link' => '//boostifythemes.com/demo/wp/zoa/',
		),
	);
...
if ( ! function_exists( 'zoa_layout5_customize' ) ) {
	function zoa_layout5_customize() {
		$menu_layout = zoa_menu_slug();
		if ( 'layout-5' === $menu_layout ) {
...
add_action( 'zoa_before_content', 'zoa_layout5_customize', 20 );
...
if ( ! function_exists( 'zoa_custom_admin_notice' ) ) {
	function zoa_custom_admin_notice() {
		$user_id = get_current_user_id();
	    if ( ! get_user_meta( $user_id, 'zoa_notice_dismissed' ) ) {
		?>
			<div class="notice notice-info" style="position: relative;">
				<p><?php
					echo sprintf(
						__( 'Introduce official Zoa Facebook community group for all users. <a href="%s">Say Hi Now!</a>', 'zoa' ),
		                'https://www.facebook.com/groups/413663839168030/'
		            );
				?>
				</p>
				<a href="?dismiss-zoa-notice" class="notice-dismiss zoa-dismiss-notice" style="text-decoration: none;"></a>
			</div>
		<?php
		}
	}
...
if ( ! function_exists( 'zoa_dismiss_admin_notice' ) ) {
	function zoa_dismiss_admin_notice() {
	    $user_id = get_current_user_id();
	    if ( isset( $_GET['dismiss-zoa-notice'] ) ) {
	    	add_user_meta( $user_id, 'zoa_notice_dismissed', 'true', true );
	    }
	}
}
add_action( 'admin_init', 'zoa_dismiss_admin_notice' );

// Core element.
add_action( 'zoa_theme_core_element', 'zoa_product_action', 10 );
add_action( 'zoa_theme_core_element', 'zoa_search_form', 20 );
add_action( 'zoa_theme_core_element', 'zoa_after_footer', 30 );
